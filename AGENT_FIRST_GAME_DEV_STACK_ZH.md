# Agent First Game Dev Stack (AFGDS)

**Agent First Game Dev Stack** 是一个基于 BMAD 方法论的端到端游戏开发全栈解决方案，旨在实现“Agent 主导，人类导演”的游戏开发范式。
...
### 2. 为什么要使用 Open GDD Template?
AI Agent 之间的协作需要一种通用的"语言"或"协议"。
*   **标准化上下文**: 传统的 GDD 格式千差万别，AI 难以精准提取信息。Open GDD Template 提供了一套结构化、语义明确的标准，确保所有 Agent（无论是策划 Planner 还是程序 Builder）都能基于同一套理解工作。
*   **消除歧义**: 它是连接"模糊创意"与"精确工程"的桥梁。通过结构化的模板，我们将自然语言的模糊性转化为工程所需的确定性输入。

### 3. 为什么要使用 BMAD?
BMAD (Breakthrough Method of Agile AI-Driven Development) 是专为 Agent-first 设计的生产级开发框架。
*   **模拟真实团队**: 不同于单一的 AI 编码助手，BMAD 模拟了真实游戏工作室的组织架构（产品经理、架构师、开发者、QA），能够处理复杂的任务依赖关系。
*   **确定性工程**: "Vibe Coding" (凭感觉写代码) 只能产生 Demo。BMAD 强调工程的严谨性，支持 Unity、Unreal 等专业引擎，确保产出的是可维护、可扩展的商业级项目，而非一次性玩具。
...
1.  **上下文窗口限制 (Context Window)**
    ...
    *   **对策**: 我们的三层架构将任务解耦。GDD Generator 负责切分设计模块，BMAD 负责模块化实现，避免将整个项目塞入 Context。
...
3.  **复杂逻辑推理 (Complex Reasoning)**
    ...
    *   **对策**: **OpenGDD Template** 强制要求结构化的逻辑描述，配合 Human-in-the-loop (人在回路) 的审查机制，确保核心逻辑的正确性。
...
**组件**: [Open Game Design Document Template](https://opengdd.borninsea.com/)
*   **定位**: 标准制定者 (Standard)
...
**组件**: [GDD Generator](https://opengdd.borninsea.com/opengdd-bmad-gdd-generator) (本仓库)
*   **定位**: 设计侧执行者 (Planner Agents)
*   **作用**: 利用 OpenGDD 标准，部署 29 个专业 Agent（涵盖策划、数值、叙事、美术指导等）将模糊的创意转化为确定性的、工程级的 GDD 文档。
*   **输出**: Development-Ready Specifications (开发就绪规格书)。
*   **工具链解耦 (Universal Interoperability)**: 生成的 GDD 是通用 Markdown 文件。即使你不使用我们的 BMAD Game Dev 构建层，也可以将这些高质量文档直接投喂给 **v0, Lovable, Bolt.new** 或其他 AI IDE (Cursor, Windsurf, Trae)，实现无缝对接。
*   **核心价值**: 确保设计意图在不同 Agent 之间无损传递。
### 3. 构建层 (Builder Layer)
**组件**: [BMAD Game Dev (BMGD)](https://opengdd.borninsea.com/opengdd-bmad-game-dev)
*   **定位**: 工程侧执行者 (Builder Agents)
*   **作用**: 读取由 GDD Generator 生成的标准化文档，通过 BMAD 框架指挥工程 Agents 进行实际的代码编写、资源集成和构建。
*   **核心能力**:
...
2.  **生成规格**: **GDD Generator** (规划层) 根据创意生成标准化的 OpenGDD 文档。
3.  **构建实现**: **BMAD Game Dev** (构建层) 读取文档，自动生成代码和资产。
4.  **迭代优化**: 人类基于结果反馈，Agent 自主进行下一轮迭代。

---
*Agent First Game Dev Stack - Empowering the Next Generation of Game Creation.*
